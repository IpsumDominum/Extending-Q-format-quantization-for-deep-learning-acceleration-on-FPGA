...
ROUND_NUMBER = 0.125/2
output_bit_num = 0
output_bit_amount = 0
def get_max(item):
    global output_bit_num,output_bit_amount
    ...
    output_bit_num += dim
    output_bit_amount += 1
    print(output_bit_num/output_bit_amount)
    return dim
class Quantize(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        global TEST_WEIGHTS
        if(not TEST_WEIGHTS and TEST_ACTIVATION):
            get_max(x)
        if(USE_Q):
...
class SelfAttention(nn.Module):
    ...
    def quantize_weights(self, module):
        if isinstance(module, nn.Linear):
            module.weight.data = Quantize.apply(module.weight.data)
            module.bias.data = Quantize.apply(module.bias.data)
    def forward(self, query_in, key_in, value_in):
        ...
        attention_weights = Quantize.apply(attention_weights)
        ...
        attention_weighted_value = Quantize.apply(attention_weighted_value)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, residual_x):        
        ...
        add_and_norm = Quantize.apply(add_and_norm)
        return add_and_norm
...
class LayerNorm(nn.Module):
    ...
    def __init__(self, features, eps=1e-6):
        ...
        self.eps = eps
    def quantize_weights(self, module):
        module.scale.data = Quantize.apply(module.scale.data)
        module.shift.data = Quantize.apply(module.shift.data)
    def forward(self, x):
        self.apply(self.quantize_weights)        
        mean = x.mean(-1, keepdim=True)
        std = x.std(-1, keepdim=True)
        mean = Quantize.apply(mean)
        std = Quantize.apply(std)
        div = Quantize.apply((std) + self.shift) + self.eps
        return Quantize.apply(self.scale * (x - mean) / (div))
...
class VocabLogits(nn.Module):
    ...
    def forward(self, x):
        x = Quantize.apply(x)
        return F.log_softmax(Quantize.apply(self.linear(x)), dim=-1)
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        if(USE_Q):
            embed = Quantize.apply(self.lut(x)) * (round(math.sqrt(self.embed_dim)/ROUND_NUMBER)*ROUND_NUMBER)
        else:
            embed = Quantize.apply(self.lut(x)) * math.sqrt(self.embed_dim)
        return Quantize.apply(Quantize.apply(embed) + Quantize.apply(self.pos_encode(embed)))
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        x = Quantize.apply(x)
        x = x + Quantize.apply(Variable(self.pe[:, : x.size(1)], requires_grad=False))
